...
const run = (body: unknown) => updateRepNameUseCase({
    shopId: 1, userId: 2, body: updateRepNameBodySchema.parse(body),
});
...
describe("代表者氏名の身分証アップロード", () => {
    it("multipartの表裏ファイルが検証を通り、氏名と同じtransactionで保存される", async () => {
        const app = express();
        app.patch("/", ...parseMultipartBody([
            { name: "frontIdCard", maxCount: 1 }, { name: "rearIdCard", maxCount: 1 },
        ]), validateBody(updateRepNameBodySchema), async (req, res) => {
            await run(req.validatedBody);
            res.sendStatus(200);
        });
        await request(app).patch("/").field(name)
        ...
        expect(mocks.updateIdCardS3Metadata).toHaveBeenCalledWith({
            idCard: shop.IdCard, data: { front_s3_metadata_id: 50, rear_s3_metadata_id: 60 }, transaction,
        });
        expect(mocks.updateName).toHaveBeenCalledWith({
            name: shop.RepresentativeName,
            data: { sei: name.sei, mei: name.mei, sei_kana: name.seiKana, mei_kana: name.meiKana }, transaction,
        });
    ...
    it("表面だけを差し替え、裏面を維持する", async () => {
        ...
        expect(mocks.updateIdCardS3Metadata).toHaveBeenCalledWith({
            idCard: shop.IdCard, data: { front_s3_metadata_id: 50, rear_s3_metadata_id: 40 }, transaction,
        });
        expect(mocks.deleteS3Metadata).toHaveBeenCalledExactlyOnceWith({ s3Metadata: shop.IdCard.FrontIdCard, transaction });
    });
    ...
    it("身分証未登録の場合は作成してショップへ紐付ける", async () => {
        ...
        expect(mocks.updateShopIdCard).toHaveBeenCalledWith(expect.objectContaining({ data: { idcard_id: 70 }, transaction }));
    });
    ...
    it("所有しないショップ・画像はアップロード前に拒否する", async () => {
        ...
        await expect(run({ ...existingBody, frontS3MetadataId: "999" })).rejects.toMatchObject({ code: "S3_METADATA_NOT_FOUND" });
        expect(mocks.uploadS3Object).not.toHaveBeenCalled();
    ...
    it("裏面のアップロード失敗時はアップロード済みの表面を補償削除する", async () => {
        ...
        mocks.uploadS3Object.mockResolvedValueOnce({ bucketName: "verification-documents", objectKey: "front", versionId: "v1", etag: null }).mockRejectedValueOnce(error);
        await expect(run({ ...name, frontIdCard: file, rearIdCard: file })).rejects.toBe(error);
        expect(mocks.deleteS3Object).toHaveBeenCalledExactlyOnceWith({ bucketName: "verification-documents", objectKey: "front", versionId: "v1" });
    ...
    it("画像不足・不正IDを拒否し、別APIの旧入力形式を維持する", () => {
        for (const body of [name, { ...existingBody, frontS3MetadataId: "0" }, { ...existingBody, rearS3MetadataId: "1.5" }]) {
            expect(updateRepNameBodySchema.safeParse(body).success).toBe(false);
        }
        expect(repNameBodySchema.safeParse({ ...name, frontFileName: "front.jpg", idFrontUpload: true }).success).toBe(true);
    });
});
