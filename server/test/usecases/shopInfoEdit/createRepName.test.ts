...
vi.mock("../../../src/services/shopInfoEdit/command.js", () => ({ createShopEditWithIdCard: mocks.createShopEditWithIdCard }));
...
const metadata = { bucket_name: "verification-documents", version_id: "old-version", content_type: "image/jpeg", original_file_name: "old.jpg" };
const shop = {
    IdCard: {
        id: 20,
...
const run = (body: unknown) => createShopEditRepNameUseCase({
    shopId: 1, userId: 2, body: createShopEditRepNameBodySchema.parse(body),
});

beforeEach(() => {
    vi.resetAllMocks();
    mocks.transaction.mockImplementation(async (callback: (t: typeof transaction) => Promise<void>) => callback(transaction));
    mocks.getMyShopHasRepName.mockResolvedValue(shop);
    mocks.uploadS3Object.mockImplementation(async ({ bucketName, objectKey }) => ({
        bucketName, objectKey, versionId: "v1", etag: "etag",
    }));
...
describe("代表者氏名変更申請の身分証アップロード", () => {
    it("multipartの画像をアップロードし、身分証・氏名・変更申請を同じtransactionで作成する", async () => {
        const app = express();
        app.post("/", ...parseMultipartBody([
            { name: "frontIdCard", maxCount: 1 }, { name: "rearIdCard", maxCount: 1 },
        ]), validateBody(createShopEditRepNameBodySchema), async (req, res) => {
            await run(req.validatedBody);
            res.sendStatus(200);
        });
        await request(app).post("/").field(name)
            .attach("frontIdCard", file.buffer, "front.jpg")
            .attach("rearIdCard", file.buffer, "rear.jpg").expect(200);
        expect(mocks.uploadS3Object).toHaveBeenCalledTimes(2);
        expect(mocks.uploadS3Object).toHaveBeenCalledWith(expect.objectContaining({
            bucketName: "verification-documents", body: file.buffer, contentType: "image/jpeg",
        }));
        expect(mocks.createS3Metadata).toHaveBeenCalledWith({ data: {
            bucket_name: "verification-documents", object_key: expect.stringContaining("idcard/shop-edit/front/1/"),
            version_id: "v1", original_file_name: "front.jpg", content_type: "image/jpeg", file_size: 5, etag: "etag",
        }, transaction });
        expect(mocks.createIdCard).toHaveBeenCalledWith({
            data: { front_s3_metadata_id: 50, rear_s3_metadata_id: 60 }, transaction,
        });
        expect(mocks.createNameShop).toHaveBeenCalledWith({
            data: { sei: name.sei, mei: name.mei, sei_kana: name.seiKana, mei_kana: name.meiKana, shop_type: "representative" }, transaction,
        });
        expect(mocks.createShopEditWithIdCard).toHaveBeenCalledWith({
            data: { user_id: 2, shop_info_id: 1, idcard_id: 70, name_representative_id: 80 }, transaction,
        });
    ...
    it("既存画像を申請専用に複製し、既存データは保持する", async () => {
        ...
        expect(mocks.getS3Object).toHaveBeenCalledWith({ bucketName: metadata.bucket_name, objectKey: "old-front", versionId: "old-version" });
        expect(mocks.uploadS3Object).toHaveBeenCalledTimes(2);
        expect(mocks.createIdCard).toHaveBeenCalledWith({ data: { front_s3_metadata_id: 50, rear_s3_metadata_id: 60 }, transaction });
        expect(mocks.deleteS3Object).not.toHaveBeenCalled();
    });

    it("表面だけを新規アップロードし、裏面は既存画像から複製する", async () => {
        await run({ ...existingBody, frontIdCard: file });
        expect(mocks.getS3Object).toHaveBeenCalledExactlyOnceWith({ bucketName: metadata.bucket_name, objectKey: "old-rear", versionId: "old-version" });
        expect(mocks.uploadS3Object).toHaveBeenCalledTimes(2);
    });
    ...
    it("所有しないショップ・画像をS3アクセス前に拒否する", async () => {
        ...
        for (const body of [{ ...existingBody, frontS3MetadataId: "999" }, { ...existingBody, rearS3MetadataId: "999" }]) {
    ...
    it("裏面アップロード失敗時は表面の新規オブジェクトを補償削除する", async () => {
        ...
        mocks.uploadS3Object.mockResolvedValueOnce({ bucketName: "verification-documents", objectKey: "new-front", versionId: "v1", etag: null }).mockRejectedValueOnce(error);
        await expect(run({ ...name, frontIdCard: file, rearIdCard: file })).rejects.toBe(error);
        expect(mocks.deleteS3Object).toHaveBeenCalledExactlyOnceWith({ bucketName: "verification-documents", objectKey: "new-front", versionId: "v1" });
        expect(mocks.transaction).not.toHaveBeenCalled();
    ...
    it("画像不足・不正ID・旧署名付きURL用入力を拒否する", () => {
        for (const body of [name, { ...existingBody, frontS3MetadataId: "0" }, { ...existingBody, rearS3MetadataId: "1.5" }, { ...name, frontFileName: "front.jpg", idFrontUpload: true }]) {
